...
class MRIImage:
    ...
    def extract_roi(self, roi_labels: Union[int, list]) -> "MRIImage":
        """
        Extract one or multiple ROIs from a segmentation image.
        
        This method is only available for segmentation images. It creates a binary
        mask where voxels belonging to the specified label(s) are set to 1, and
        all others to 0.

        Parameters
        ----------
        roi_labels : int or list of int
            Label ID or list of label IDs to extract (e.g., 17 for left hippocampus,
            or [17, 53] for bilateral hippocampus).

        Returns
        -------
        MRIImage
            A binary mask MRIImage with image_type='binary_mask' where selected 
            ROIs = 1, others = 0.

        Raises
        ------
        ValueError
            If the image is not a segmentation image.
        ...
        >>> left_hippo = seg.extract_roi(17)
        >>> 
        >>> # Extract bilateral hippocampus (labels 17 and 53)
        >>> bilateral_hippo = seg.extract_roi([17, 53])
        >>> 
        >>> # Extract multiple subcortical structures
        >>> subcortical = seg.extract_roi([10, 11, 12, 49, 50, 51])
        >>> 
        >>> # Visualize the extracted ROI
        >>> bilateral_hippo.show()
        >>> 
        >>> # Use as a mask on anatomical image
        >>> t1 = MRIImage.load('T1.mgz')
        >>> masked_t1 = t1.apply_mask(bilateral_hippo.data > 0)

        Notes
        -----
        Common FreeSurfer label IDs:
        - Left-Hippocampus: 17
        ...
        See FreeSurfer's FreeSurferColorLUT.txt for complete label list.
        """
        # Check if image is segmentation type
        if self.image_type != 'segmentation':
            raise ValueError(
                f"extract_roi() can only be used on segmentation images. "
                f"Current image type is '{self.image_type}'. "
                f"Load the image with: MRIImage.load(path, image_type='segmentation')"
            )
        
        # Ensure roi_labels is a list
        if not isinstance(roi_labels, (list, tuple, np.ndarray)):
            roi_labels = [roi_labels]
        
        # Create binary mask for the specified ROI(s)
        roi_mask = np.isin(self.data, roi_labels).astype(np.uint8)
        
        # Create new MRIImage with binary_mask type
        roi_image = MRIImage(
            data=roi_mask,
            affine=self.affine.copy(),
            header=self.header.copy() if self.header else None,
            filepath=None,  # This is a derived image, not from disk
            image_type='binary_mask'
        )
    ...
    def get_label_stats(self) -> dict:
        """
        Get statistics about labels in a segmentation image.
        
        Returns a dictionary with information about each unique label,
        including the number of voxels and volume.

        Returns
        -------
        dict
            Dictionary with label IDs as keys, each containing:
            - 'n_voxels': number of voxels with this label
            - 'volume_mm3': volume in mm³ (if affine available)
        ...
        --------
        >>> seg = MRIImage.load_segmentation('aparc+aseg.mgz')
        >>> stats = seg.get_label_stats()
        >>> print(f"Left hippocampus voxels: {stats[17]['n_voxels']}")
        >>> print(f"Left hippocampus volume: {stats[17]['volume_mm3']:.2f} mm³")
        """
        if self.image_type != 'segmentation':
            raise ValueError(
                f"get_label_stats() can only be used on segmentation images. "
                f"Current image type is '{self.image_type}'."
            )
        ...
        for label_id in unique_labels:
            n_voxels = np.sum(self.data == label_id)
