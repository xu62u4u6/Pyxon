...
def load_lut_csv(path: str) -> Dict[str, str]:
    """
    Load a FreeSurfer-style LUT CSV into a color dictionary (ID → hex color).

    Parameters
    ----------
    path : str
        Path to LUT CSV file, expected columns ['ID', 'Name', 'R', 'G', 'B', 'A'].

    Returns
    -------
    dict
        Mapping of label ID (as str) to hex color (e.g., '#4682b4')
    """
    df = pd.read_csv(path)
    colors = df[['R', 'G', 'B']].to_numpy() / 255.0
    color_dict = {str(row['ID']): mcolors.to_hex(rgb) 
                  for row, rgb in zip(df.to_dict('records'), colors)}
    return color_dict
...
class ColorManager:
    ...
    # === MRI colormaps ===
    FREESURFER_CMAP = load_lut_csv(_DATA_DIR / "LUT.csv")
    FASTSURFER_CMAP = load_lut_csv(_DATA_DIR / "LUT.csv")
    BINARY_CMAP = {"0": "#000000", "1": "#FFFFFF"}

    # === 合併所有預設 cmap ===
    predefined_cmaps = {
        # MRI colormaps
        'binary': BINARY_CMAP,
        'freesurfer': FREESURFER_CMAP,
        'fastsurfer': FASTSURFER_CMAP,
    }
